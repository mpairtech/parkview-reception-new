      ...
      <div class="row px-3">
        <div class="col-lg-12">
          <p class="m-0 fs-6 text-dark mt-2 px-4 py-2">
            <strong>Choose Your Appointment Schedule {{update}}</strong>
          </p>
          <div v-for="doc in docInfo" class="row border-bottom px-4">
            <div class="col-lg-6 my-auto">
              <p class="fw-bold m-0">{{doc.doctor}}</p>
              <p class="m-0 designation-font">{{doc.qualification}} ( {{doc.department}} )</p>
              <p class="m-0 designation-font">Associate Professor, {{doc.department}}</p>
          ...
          <div class="row my-5">
            <div class="col-lg-8 mx-auto">
              <div class="appointScheList" style="height: 25rem">
                <button v-for="index in Number(max)" @click="handleValue(index)" class="btn w-100 border-0 p-0 my-2" :disabled="this.reserved.includes(','+index.toString()+',') ? true : false">
                  <div class="row text-dark fw-light text-center p-0 mb-0">
                    <div class="col-lg-3">
                      <p class="border yellow-color-light py-2 mb-0">{{index}}</p>
                    </div>
                    <div class="col-lg-9">
                      <p class="border yellow-color-light py-2 mb-0">
                        {{addMinutes(Number(min)*(index-1))}} - {{addMinutes(Number(min)*index)}}
                      </p>
                    </div>
                  </div>
                </button>
...
<script>
  export default{
    name:'Schedule',
    props: ["active","docId","max","min","setData","visitDate","day","session","update","setupdate","close"],
    data(){
      return{
        appointments:[],
        reserved:"",
    ...
    methods:{
      ...
      getDocById(){
        ...
        fetch("http://server.parkviewappointment.com/parkview/reception/getDoctorById", {
          method: "POST",
          body: data
        })
          .then((res) => res.json())
      ...
      getSerial(){
          ...
          fetch("http://server.parkviewappointment.com/parkview/reception/getSerial", {
            method: "POST",
            body: data
          })
            .then((res) => res.json())
    ...
    getDocAppointment() {
      ...
      data.append("day",this.day)
      data.append("session",this.session)
      fetch('http://server.parkviewappointment.com/parkview/reception/getDocAppointment', {
        method: 'POST',
        body: data,
      })
        .then((res) => res.json())
    ...
    handleValue(index){
      var period = this.addMinutes(Number(this.min)*(index-1))+'-'+this.addMinutes(Number(this.min)*index)
      console.log(period)
